...
from src.pert import eval_cov_sharpness
...
def main():
    ...
    for model_name in tqdm(model_list):
        ...
        if model.pretrained_cfg["num_classes"] != 1000:
            logging.info(f"Skipping model {model_name} as it is not trained on ImageNet")
            continue
        if pretrained:
        ...
        with torch.no_grad():
            ...
            for img, target in tqdm(train_loader):
                ...
                alp = args.rho  # Noise variance
                # jacobian(model, img) # This takes 17 minutes to run
                noise = torch.randn(num_samples, *(img.shape[1:])).to(args.device) * math.sqrt(alp)
                out = model(img + noise)
    ...
    plt.annotate(f"Pearson correlation: {correlation:.2f}", xy=(0.05, 0.95), xycoords='axes fraction', fontsize=12, verticalalignment='top')
    savefig(f"res/{args.model}/{args.run_id}")
